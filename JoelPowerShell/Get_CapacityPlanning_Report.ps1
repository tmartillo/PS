...
VMWare Capacity & Performance Report
 ...
 PREREQUISITES for this script: Powershell V2, PowerCLI 6.5+, Microsoft Chart Controls for .NET Framework (download from this link: 
...
############################# CHANGELOG #######################################
 ...
 January 2017      Updated to Atlas Copco
/#>
################################ CONSTANTS ######################################
...
#-------------------------CHANGE THESE VALUES--------------------------------
$SMTPServer = "smtp.nasa.group.atlascopco.com"
$vCenterServerName = "usssusvc001.nasa.group.atlascopco.com","usssusvc010.nasa.group.atlascopco.com","ssdsbrvc0001","ssisusvc0005"
$ToAddress = "joel.teel@us.atlascopco.com"
#-----------------------------------------------------------------------------
foreach ($vCenter in $vCenterServerName) {

$ScriptVersion = "v2.0 - Community Edition" # Included in the Runtime info in $HTMLFooter
$Subject = "VMware Capacity & Performance Report for " + $vCenter
$FromAddress = (($vCenter.Substring(9,3)).ToUpper()) + "`_Monthly_Capacity_Report" + "@us.atlascopco.com"

$ColorArray = "Red", "Orange", "Purple", "Blue", "Olive", "SlateGrey", "Orange", "Purple", "Blue", "Olive"
$ColorArrayIndex = 0

$HTMLHeader = "<HTML><TITLE> VMware Capacity & Performance Report for " + $vCenter + "</TITLE>"
...
Connect-VIServer $vCenter -Credential $credadm
...
$HTMLBody = "<BODY>"
$HTMLBody += "$Date<br>"
$HTMLBody += "<h1><b><center>VMWare Capacity & Performance Report for " + $vCenter + "</center></b></h1>"

$HTMLBody += CreateHeader ("INVENTORY FOR <font color=green><b> vCENTER " + $vCenter + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("CPU CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenter + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("MEMORY CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenter + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("DATASTORE CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenter + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("VIRTUAL MACHINE REPORT FOR <font color=green><b> vCENTER " + $vCenter + "</font></b>")
...
Clear-Variable -name Attachments , Clusters , ClusterTemp, Datastores, DatastoresInCluster, Date, DC, FromAddress, hosts, HTMLHeader, HTMLFooter, HTMLBody, HTMLPage, NumberOfVMHostsInCluster, Snapshots, Subject, Templates, VM, VMHosts, VMHostsInCluster, VMsInCluster
